...
- **Поиск сущностей (25+ категорий)**:

  **Документы РФ:**
  - Паспорт РФ (`RU_PASSPORT`) → `<PASSPORT_RF>`
  - Загранпаспорт (`RU_INT_PASSPORT`) → `<PASSPORT_INT>`
  - ИНН (`RU_INN`) → `<INN>`
  - СНИЛС (`RU_SNILS`) → `<SNILS>`
  - Водительские удостоверения (`RU_DRIVER_LICENSE`) → `<DRIVER_LICENSE>`
  - Полис ОМС (`RU_OMS`) → `<OMS>`
  - Гос. номера авто (`RU_VEHICLE_PLATE`) → `<CAR_PLATE>`

  **Контакты:**
  - ФИО (`PERSON`) → `<PERSON>`
  - Телефонные номера (`PHONE_NUMBER`) → `<PHONE>`
  - Email адреса (`EMAIL_ADDRESS`) → `<EMAIL>`
  - Telegram Chat ID (`TG_CHAT_ID`) → `<TG_CHAT_ID>`

  **Цифровые следы:**
  - IP-адреса (`IP_ADDRESS`) → `<IP>`
  - MAC-адреса (`MAC_ADDRESS`) → `<MAC>`
  - IMEI устройств (`EME_IMEI`) → `<IMEI>`

  **Финансы:**
  - Банковские карты (`CREDIT_CARD`) → `<BANK_CARD>`
  - IBAN счета (`IBAN_CODE`) → `<BANK_ACCOUNT>`
  - Криптокошельки (`CRYPTO`) → `<WALLET>`
  - CVV коды (`CVV`) → `<CVV>`

  **Геоданные и Время:**
  - GPS координаты (`GPS_COORDS`) → `<GEO>`
  - Даты (`DATE_TIME`) → `<DATE>`
  - Локации (`LOCATION`) → `<LOC>`

  **Организации и Группы:**
  - Организации (`ORGANIZATION`) → `<ORG>`
  - Национальности/Религии (`NORP`) → `<GROUP>`
